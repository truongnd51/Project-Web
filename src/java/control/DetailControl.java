...
public class DetailControl extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        List<Feedback> listF = dao.getFeedbackByPid(id);
         List<Product> listp = dao.getProductNew();
        Product p = dao.getProductById(id);
        List<Product> listById=dao.getProductByCId1(p.getCategory());
        if(name!= null && id!= null&& comment!= null){
        dao.addFeedback(name, comment, id);} 
       
        request.setAttribute("detail", p);
        request.setAttribute("id", id);
        request.setAttribute("listP", listp);
        request.setAttribute("listF", listF);
        request.setAttribute("listC", listById);
